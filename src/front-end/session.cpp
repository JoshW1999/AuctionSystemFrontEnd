...
/*
    ...
    Output: None
*/
Session::Session(string userAccount, string availableItems)
{
...
Item Session::getItem(std::string itemname)
{
    for (auto i = this->availableItems.begin(); i != this->availableItems.end(); i++)
    {
...
void Session::logout()
{
    ...
    { //Syntax to catch all sorts of exception
...
void Session::writeDailyTransaction()
{
    ...
    // cout << to_string(1 + ltm->tm_hour) + to_string(1 + ltm->tm_min) + to_string(1 + ltm->tm_sec) << endl;

    string filename = "../../../../resources/daily_transactions/dt";
    filename += to_string(now) + ".txt";
...
void Session::readAvailableItems()
{
    ...
    // std::cout << this->availableItems.size() << endl;
    string in_string;
    while (getline(in, in_string))
    {
        ...
        Item item;
        if(itemInfo.size()==4){
            item =  Item(itemInfo[0], itemInfo[1], "", stoi(itemInfo[3]), stof(itemInfo[2]));
        }
        else if(itemInfo.size()==5){
